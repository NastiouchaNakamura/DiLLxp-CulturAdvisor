...
def fetch() -> List[Resource]:
    ...
    for record in response["records"]:
            ...
            if "sous_domaines" in record["fields"]:
                resource.fields["sous_domaine"] = record["fields"]["sous_domaines"]
            ...
            if "code_insee_arrondt" in record["fields"]:
                resource.fields["code_insee"] = record["fields"]["code_insee_arrondt"]
            
            # code insee
            if "code_insee" in record["fields"]:
                resource.fields["code_insee"] = record["fields"]["code_insee"]
             
            # code insee EPCI
            if "code_insee_epci" in record["fields"]:
                resource.fields["code_insee_EPCI"] = record["fields"]["code_insee_epci"] 
            
            # type equipement lieux
            if "type_equipement_ou_lieu" in record["fields"]:
                resource.fields["type_equipement_lieux"] = record["fields"]["type_equipement_ou_lieu"]
             
            # precision equipement
            if "precision_equipement" in record["fields"]:
                resource.fields["precision_equipement"] = record["fields"]["precision_equipement"]
            ...
            if "adresse_postale" in record["fields"]:
                resource.fields["adresse_postal"] = record["fields"]["adresse_postale"]
            ...
            if "identifiant_origine" in record["fields"]:
                resource.fields["identifiant_origine"] = record["fields"]["identifiant_origine"]
             
            # num departement
            if "n_departement" in record["fields"]:
                resource.fields["num_departement"] = record["fields"]["n_departement"]
             
            # num region
            if "n_region" in record["fields"]:
                resource.fields["num_region"] = record["fields"]["n_region"]
            ...
            if "type_de_cinema" in record["fields"]:
                resource.fields["type_cinema"] = record["fields"]["type_de_cinema"]
              
            # nb fauteils cinema
            if "nombre_fauteuils_de_cinema" in record["fields"]:
                resource.fields["nb_fauteils_cinema"] = record["fields"]["nombre_fauteuils_de_cinema"]
              
            # nb ecrans
            if "nombre_ecrans" in record["fields"]:
                resource.fields["nb_ecrans"] = record["fields"]["nombre_ecrans"]
              
            # nb salles theatre
            if "nombre_de_salles_de_theatre" in record["fields"]:
                resource.fields["nb_salles_theatre"] = record["fields"]["nombre_de_salles_de_theatre"]
            
            # jauge theatre
            if "jauge_du_theatre" in record["fields"]:
                resource.fields["jauge_theatre"] = record["fields"]["jauge_du_theatre"]

            # code reseau bibliotheque
            if "code_du_reseau_de_bibliotheques" in record["fields"]:
                resource.fields["code_reseau_bibliotheque"] = record["fields"]["code_du_reseau_de_bibliotheques"]

            # nom reseau bibliotheque
            if "nom_du_reseau_de_bibliotheques" in record["fields"]:
                resource.fields["nom_reseau_bibliotheque"] = record["fields"]["nom_du_reseau_de_bibliotheques"]

            # archeologie detail
            if "archeologie_detail" in record["fields"]:
                resource.fields["archeologie_detail"] = record["fields"]["archeologie_detail"]
             
            # surface bibliotheque
            if "surface_bibliotheque" in record["fields"]:
                resource.fields["surface_bibliotheque"] = record["fields"]["surface_bibliotheque"]
            ...
            if "identifiant_deps_a_partir_de_2022" in record["fields"]:
                resource.fields["identifiant_deps"] = record["fields"]["identifiant_deps_a_partir_de_2022"]
             
            # libelle EPCI
            if "libelle_epci" in record["fields"]:
                resource.fields["libelle_EPCI"] = record["fields"]["libelle_epci"]
